...
**④ 逐股深度调研 + 真实行情复核**
两阶段出票：先初选 6–8 支，再对**每一支入选股单独跑一遍产业链八层深度调研**（与「研」按钮同源：产业链位置 / 卡点与稀缺性 / 五流派多空 / 催化与验证 / 风险 / 证伪 / 研究优先级），结论并入推荐理由（并发执行控制耗时）。随后所有价格/涨跌幅用真实行情**回填**（禁止 AI 编造数字），幻觉代码、以及**连续 6 个交易日逐日下行（6 连阴）**的标的直接剔除。每支都附：**产业链位置 / 五派分歧 / 龙虎榜资金信号 / 今日看点 / 主要风险 / 证伪条件**。
...
- **K线 / 资金流 / 筹码分布 / 技术指标**：日/周/月 K 蜡烛 + MA5/MA10 + 成交量；通达信口径 MACD/KDJ/RSI/BOLL 现值与金叉死叉；个股资金流五档（主力/超大/大/中/小单，红进绿出）；**筹码分布图**——换手率衰减 + 成本中枢高斯核近似还原各价位筹码堆积（红获利盘 / 绿套牢盘），直读获利比例 / 平均成本 / 现价。
- **AI 流式聊天 + 深度调研 + 收藏**：底栏直连 DeepSeek 逐字流式；「研」一键进产业链八层深度调研工作流；调研结论可**分组收藏**、整段或逐条一键复制，永久存本机。
- **非交易时段提示**：A 股未开盘 / 午休 / 收盘 / 周末，行情条下方红条醒目提示「沿用上一交易日数据」，避免把旧数据误当实时盘口。
...
| 行情主页 | 自选股 + AI 表盘 | 今日 AI 推荐 |
|:---:|:---:|:---:|
| ![行情主页](docs/screenshots/market.png) | ![自选股](docs/screenshots/watchlist.png) | ![今日AI推荐](docs/screenshots/recommend.png) |

| K线 + 资金流向 | 深度调研收藏 | 设置 |
|:---:|:---:|:---:|
| ![K线资金流](docs/screenshots/kline.png) | ![调研收藏](docs/screenshots/research.png) | ![设置](docs/screenshots/settings.png) |
...
- **液态玻璃外观（实验 · 设置可选）**：仿 iOS「Liquid Glass」的磨砂玻璃皮肤——半透明 + 模糊 + 高光描边 + 极光背景 + 浮动药丸 Tab，夜/日双主题适配；默认关闭，老旧安卓机型自动降级为不透明底色。
...
## 2026-06-12（第五批：三个 A 股数据维度——人气榜 + 分红历史 + 股本/市值）

> 接口设计继续参考 1nchaos/adata（Apache-2.0，仅作接口字典，未复制代码）；全部接口均经真实请求复核返回形状后才接入。

### 新增
- **🔥 人气榜 Top 10（行情页）**：同花顺个股热度小时榜（`dq.10jqka.com.cn/fuyao/hot_list_data/...`，普通 GET、免 Cookie/签名，code+name+涨跌幅+人气值+人气标签一次到位——对比东财人气榜 POST 只回 secid 还需二次行情解析，故弃用后者）。板块热力卡下方新卡：名次（前三高亮）+ 名称/代码 + 人气标签（如「3天2板」/首个概念）+ 涨跌幅，**点击任意行直接进该股K线**。北交所股票按全 app 口径过滤。5 分钟 TTL + last-good：失败沿用最近真实榜单并诚实标注时间；SQLite `hot_cache` 落盘，冷启动秒出上次真实榜单（标「缓存」）；从未成功且无缓存 → 整卡隐藏，绝不演示数据。已接入启动并发预热 / 定时保温 / 回前台保温。
- **最近分红（K线页个股详情）**：东财 `RPT_SHAREBONUS_DET` 分红明细（按预案公告日倒序，最新方案在前）。快照格下方一行「最近分红 10派X元 · 除权 YYYY-MM-DD · 股息率 x.xx%」（未实施显示进度；纯转股如「10转6.00」同样支持；"不分配不转增"行剔除）。按日缓存（`divi_<code>`），从未分红（接口 success 但无记录）也按日缓存且不显示该行。同时向 K线「AI 解读」上下文注入一行分红摘要（prompt 模板未动）。
- **股本/市值（K线页个股详情）**：东财 push2 `stock/get` 的 `f84总股本/f85流通股/f116总市值/f117流通市值 + f162动态市盈率/f167市净率`（fltt=2，实测核对 f116=f43×f84）。快照格由 9 格扩到 15 格：总市值/流通市值/总股本/流通股/市盈(动)/市净率，市值按「万亿/亿」、股本按「亿股/万股」四舍五入展示。股本按日缓存（`share_<code>`），**市值用最新现价 × 股本现算**保持实时。注意：该字段编号是 stock/get 口径，与 ulist/clist 不通用（ulist 的 f84 是小单净额），故单独请求而非并入资金流接口。AI 上下文同步注入总市值/股本/动态PE 一行。
- 三个维度的 Rust 解析器（`parse_hot_stocks` / `parse_dividends` / `parse_share_info`）均带真实形状单元测试。

### 接口与代码位置
- Rust：`extra.rs` 新增三组 解析器+fetcher（全走 `em_get_text` 免 gzip + 3 次重试客户端）；`lib.rs` 注册 `fetch_hot_stocks` / `fetch_dividends` / `fetch_share_info` 三命令。
- 前端：`api.js` 三个封装；`market.js` `renderHot`（含 `hot_cache` 冷启动回填）；`kline.js` `loadDividends`/`loadShareInfo` 按日缓存 + 详情渲染 + AI 上下文注入；`index.html` 人气榜卡 + `.hot-*`/`.sd-divi` 样式。

## 2026-06-12（第四批：三个 A 股数据维度——所属概念/板块 + 北向资金 + 历史主力资金流）

> 接口设计参考 1nchaos/adata（Apache-2.0，仅作接口字典，未复制代码）；全部接口均经真实请求复核返回形状后才接入。

### 新增
- **个股所属概念/板块（K线页）**：东财 F10 `RPT_F10_CORETHEME_BOARDTYPE`，行业/地域/概念分类，行业 chip 高亮描边；指数成分类噪音（标普/富时/MSCI/两融/茅指数/百元股等）黑名单过滤、细分行业Ⅱ/Ⅲ去重，最多展示 10 个。按日缓存（SQLite `boards_<code>`），板块隶属变化极慢，过期缓存还能作离线兜底。同时注入 K线「AI 解读」数据上下文（`所属概念/板块: …` 一行，prompt 模板未动）。
- **近5日主力净流入（K线页）**：东财 `fflow/daykline` 历史资金流（日级），列序经实测校验（主力净额=大单+超大单）。个股详情新增 5 格红绿日历式区块（日期 + 主力净额）；并向「AI 解读」上下文注入一行逐日明细 + 合计。
- **北向资金一行（行情页，市场情绪卡下方）**：⚠️ 经复核，北向"净买额"自 2024-08 交易所新规起已停止披露（push2 kamt 实时接口恒 0 / "-"，`RPT_MUTUAL_DEAL_HISTORY` 净买额字段为 null）——仍按日披露的只有**成交金额**（次日更新）。故如实展示「北向资金 MM-DD 成交 X亿（沪 a + 深 b）· 净买额自2024-08起不再披露」，绝不伪造净流入；取不到数据整行隐藏。2 小时 SQLite 缓存（`north_cache`）+ 失败沿用旧缓存。
- 三个维度的 Rust 解析器（`parse_flow_days` / `parse_stock_boards` / `parse_north_history`）均带真实形状单元测试。

> 改动：`extra.rs` / `lib.rs`（3 个新命令）+ `api.js` / `kline.js` / `market.js` / `index.html`；桌面与安卓同源生效。

## 2026-06-12（第三批：板块/情绪快照落盘 + 安卓回前台秒级保温）

### 新增
- **板块热力 / 市场情绪快照持久化（SQLite `heat_cache` / `sent_cache`）**：每次拉到真实板块、真实情绪即落盘（60 秒节流，体量极小）。冷启动（哪怕离线）与 K线缓存并发回填，**首屏直接显示上次真实板块热力与情绪表盘**，meta 诚实标注「缓存 HH:MM」（隔天显示「缓存 MM-DD HH:MM」），新数据拉到后无感替换。演示数据从此只剩「从未成功且无任何缓存」一种触发路径，真机绝不把演示当真。
- **情绪表盘 last-good**：本次拉取失败但本会话/缓存里有真实情绪 → 沿用并标「沿用/缓存 HH:MM」，与板块热力、K线的 last-good 行为对齐（此前失败会直接画演示分数）。
- **安卓回前台秒级保温（visibilitychange + focus）**：安卓把 App 切后台时会节流甚至冻结 WebView 定时器，回前台界面可能停在几分钟前。现在一回前台立即并行补一拍：指数条 + 情绪 + 板块 + 自选行情 + 两路快讯（错峰 3 秒）+ 报警补查。全部复用现有 last-good / 请求去重 / 25s 节流，请求量与正常一拍相当；5 秒防抖避免 visibilitychange 与 focus 双触发或与定时器叠加。

> 纯前端改动（main.js / market.js），无需重新编译 Rust；桌面与安卓同源生效。

## 2026-06-12（第二批：启动并发预热 + 全页面后台保温 + K线缓存落盘，切页秒开）

### 新增
- **启动并发预热**：App 打开即**并行**拉取四路主数据（指数条 / 情绪+板块 / 自选行情+AI / 两路快讯 / 推荐缩略图），不再串行 await 一路卡全队；自选页、快讯页**不等你点进去才开始加载**，切过去就是现成数据。自选相关快讯错峰 3 秒启动，避免启动瞬间请求扎堆。
- **后台保温（非当前页也保持新鲜）**：当前页仍按设置的刷新间隔实时刷；非当前页改为每 3 个间隔低频保温一次（情绪/板块与自选行情错开 1 拍）；两路快讯每 60 秒后台刷新（自选相关错峰 20 秒）。保温复用 last-good 缓存与请求去重，实际新增请求量很小，也不会加剧东财 rustls 偶发掐线。
- **K线缓存持久化（SQLite）**：共享K线缓存层的日K数据落盘（最多 40 支、每支近 120 根、写入 3 秒防抖合并），**冷启动（哪怕离线）也能立刻画出上次成功的真实K线/推荐缩略图**——配合 last-good 机制，离线时显示的是真实历史数据并标注缓存时间，绝不造假。
- **K线失败可一键重试**：K线加载失败的提示里加了「↻ 重试」按钮，点一下重拉K线 + 行情快照/资金流，不用再切周期或退出重进。
- **候选池腾讯兜底**：今日 AI 推荐的本地候选池（涨幅榜/主力净流入榜）此前只依赖东财 clist 且无重试，被掐就变薄甚至变空。现在 ① 榜单请求走「免 gzip + 3 次退避重试」客户端；② 东财两榜全空时自动切**腾讯全市场成交额榜**（取最活跃 80 支，本地按涨幅/主力净流入各排 35 支），候选池不再因东财单点故障开天窗。
- **快讯接口统一可靠性等级**：自选股逐支资讯与东财 7×24 快讯改用与板块/资金流/K线一致的重试客户端；两路快讯结果落盘（SQLite），重启秒出上次内容。

### 修复
- **真机快讯页不再闪现示例数据**：未加载到真实快讯前显示「加载中」占位（浏览器预览仍走演示数据）。

> 桌面版与手机版同源改动；后端 Rust 改动需重新编译（桌面 `cargo tauri build` / 安卓 `cargo tauri android build --apk`）。

## 2026-06-12（手机端取数可靠性大修：K线竞速三源 + 共享缓存 + last-good）

### 修复
- **「今日 AI 推荐」缩略图在手机上经常空白**：根因是 8 支推荐的近 30 日日K此前**逐支串行**拉取（一支在安卓上撞到东财掐线就要等满"3 次重试 × 3 个源"的整条链，整队排队可达几分钟），且一次失败就永远空白。现改为：**有界并发 3 支** + 每支到货**立即渐进绘制**（不等全队列）+ 失败 30 秒冷却后**自动重试**（最多 4 轮）。
- **筹码分布 / K线在手机上时常刷不出**：后端 `fetch_kline` 由"东财跑完整条重试链失败后才轮到腾讯/新浪"的串行链，改为**竞速取数**——东财（数据最全，带换手率/成交额→筹码精度最高）与"腾讯→新浪"备援链并发赛跑，东财有 2 秒先发权：东财健康时它先回、筹码最准；东财被掐（安卓 rustls close_notify/重置/超时）时备援**秒级顶上**，不再白等 30 秒+。资金流/换手率接口（东财 vs 腾讯兜底）同样改为竞速。
- **瞬时网络波动把已显示的图打成白板**：新增**共享K线缓存层**（`klinecache.js`，推荐缩略图 / K线页 / 战绩回算共用）——按"股票+周期"缓存整段K线，30/60/250 根共享同一份数据（5 分钟内命中 0 请求，重复请求合并去重）；拉取失败但有上次成功数据时返回 **last-good 旧数据**并在K线页标注「⚠ 网络波动，显示 HH:MM 缓存数据」，绝不让画过的图变白、也绝不用模拟数据冒充（真机无数据时仍是诚实的失败提示）。
- **推荐战绩回算**改为有界并发 4（此前几十支无界并发同时打接口，更易触发掐线）；K线页加载加**防过期响应**守卫（快速切股/切周期时旧响应不再覆盖新图）。

> 桌面版与手机版同源改动；后端 Rust 改动需重新编译（桌面 `cargo tauri build` / 安卓 `cargo tauri android build --apk`）。


## 2026-06-10（v1.0.1：非交易时段提示 + 推荐下行剔除）

### 新增
- **非交易时段醒目红条**：A股未开盘 / 午休 / 收盘 / 周末休市时，行情条下方常驻红色提示条；暂未开盘与休市会注明「资金流向 / 板块热力等沿用上一交易日数据」，避免误把旧数据当实时盘口。每 30 秒自动复核时段。
- **今日 AI 推荐新规则——连续 6 日逐日下行者剔除**：对每支候选，看最近 6 个交易日的真实收盘价，**只有当这 6 天每一天都严格低于前一天（6 连阴式下跌）时才直接剔除**；中间只要有任意一天持平或上涨就保留。判定需要 7 个收盘价（6 个日间变动全部下跌）；数据不足时不判定（保留）。

- **AI 数据注入层（伪对话注入）+ 盘面技术解读**：K线页「AI 解读」是**独立的盘面技术视角**（筹码结构 / MACD-KDJ-RSI-BOLL 信号 / 量价位置 / 支撑压力 / 短线研判 / 失效信号），与自选股小仪表盘的"产业链基本面打分"分工互补。两者都把**真实筹码/指标/行情作为"已查到的事实"伪对话注入**并追加防幻觉规则；分析模板/系统提示一字不动（借鉴 go-stock）。
- **K线三源兜底（东财→腾讯→新浪）**：此前 K线/筹码只依赖东方财富一家，东财在手机上一抖就整页空白。现增加**腾讯、新浪**作为降级备源——东财失败自动切腾讯（前复权日/周/月）、再切新浪（日K），价格始终真实。备源不含换手率/成交额时，筹码退化为成交量分布但价位正确。
- **K线/筹码在手机上更稳**：K线接口改用与板块/资金流一致的「免 gzip + 3 次退避重试」客户端，化解安卓 rustls 对东财偶发的 close_notify/连接重置——筹码分布、换手率、K线在手机上"刷不出来"的概率大幅下降。
- **快讯升级财联社电报**：7×24 快讯主源切换为**财联社电报**（资讯更快更全、带关联个股/板块），东财 7×24 作失败兜底；配合本地情绪词典，每条即时标利好/利空。
- **通达信口径技术指标（MyTT）**：K 线页显示 MACD / KDJ / RSI / BOLL 现值与**金叉/死叉**状态，采用通达信口径（中国式 SMA，250 根收敛），数值与通达信/同花顺/雪球对齐。（移植自 mpquant/MyTT，MIT，见 NOTICE。）
- **涨跌报警 + 交易日历**：设置页可开启「涨跌报警」并设阈值，自选股当日涨跌幅达标即弹提醒（仅交易日、每股 5 分钟去重，纯本地）。非交易时段红条改用**真实节假日日历**（chinese-days，内置 2025–2026 + 在线更新），国庆/春节等休市也能正确识别，不再只看周末。
- **快讯本地情绪标（零成本）**：7×24 快讯每条用本地关键词加权词典即时打分，贴「利好（红）/ 利空（绿）」小标——纯本地计算，不调用 AI、不耗 token。（思路借鉴 go-stock 情绪分析，词典自编，见 NOTICE。）
- **筹码分布图（K线页）**：每只股票的 K 线页新增「筹码分布」面板——基于近 250 日日 K + 换手率衰减 + 成本中枢高斯核，近似还原各价位的筹码堆积（红＝获利盘 / 绿＝套牢盘），并直读「获利比例 / 平均成本 / 现价」。同花顺式高级观感，纯本地计算无需私有接口。（算法移植自 go-stock，Apache-2.0，见 NOTICE。为此 K 线接口补取了换手率/成交额两列。）
- **液态玻璃外观（设置可选）**：设置页新增「液态玻璃外观」开关，勾选后界面切换为仿 iOS 26「Liquid Glass」的磨砂玻璃质感——半透明 + 模糊 + 高光描边 + 极光背景 + 浮动药丸 Tab，夜/日双主题皆适配；默认关闭，老旧安卓机型自动降级为不透明底色。（注：受 WebView 限制，为 CSS backdrop-filter 近似，非苹果原生 Metal 实时折射。）
- **今日 AI 推荐升级为「两阶段·逐股深度调研」**：第一阶段先用真实行情候选池 + 五流派打分初选 6–8 支；第二阶段对**每一支入选股**再单独跑一遍**产业链深度调研**（复用「研」按钮同源的产业链八层方法论：产业链位置 / 卡点与稀缺性 / 五流派多空 / 催化与验证 / 风险 / 证伪 / 研究优先级），结果并入该股推荐理由。并发执行控制耗时；单股深调失败只缺该股深调段、不影响整体。生成时间相应变长（约 3–6 分钟）。
- **调研收藏一键复制**：每条收藏右上角加「复制」，每个分组标题加「复制全部」（整组按主题分隔合并到剪贴板）；配合上一版放开的文字选中，手机上不用再手动长按选长文。

### 修复
- **板块热力 / 资金流偶发"请求失败"**（手机版尤甚，切走页面再回来易触发）：① 后端东财请求加 3 次退避重试，化解 rustls 偶发的 `close_notify` / 连接重置；② 资金流接口改用与板块一致的免 gzip 客户端（此前默认客户端带 gzip，更易触发该问题）；③ 前端板块热力加 25s 节流 + 请求在途守卫 + **last-good 缓存**：单次失败时沿用上一次的真实数据（标注"沿用 时:分:秒"），不再瞬间打回演示数据 / 报错。
