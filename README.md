...
- **AI 数据注入层（伪对话注入）**：K线页新增「AI 解读」——把已算好的**筹码（获利比例/平均成本）、通达信指标（MACD/KDJ/RSI/BOLL 金叉死叉）、实时行情**作为"已查到的事实"注入个股分析，并追加防幻觉强制规则。分析模板/系统提示一字不动，只补数据上下文（借鉴 go-stock 伪对话注入），让 AI 结论有真实数据支撑、少编造。
