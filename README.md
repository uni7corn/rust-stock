...
- **今日 AI 推荐新规则——近 6 日整体趋势下行者剔除**：对每支候选，取最近 6 个交易日的真实收盘价做**最小二乘线性回归**，若拟合直线**斜率为负（即 6 日整体趋势向下）**，则直接从推荐列表剔除。判定的是**整体趋势**而非逐日涨跌——只要 6 日连线整体向下倾斜即算下行，中间允许个别交易日反弹（不要求每天都比前一天低）。数据不足 2 个收盘价时不判定（保留）。
